...
privileged aspect UsersController_Roo_Controller {
    ...
    public String UsersController.create(@Valid Users users, BindingResult result, Model model, HttpServletRequest request) {
        ...
        users.persist();
        return "redirect:/users/" + encodeUrlPathSegment(users.getId().toString(), request);
    }
    ...
    public String UsersController.findUsersesByUsernameForm(Model model) {
        return "users/findUsersesByUsername";
    }
    
    @RequestMapping(params = "find=ByUsername", method = RequestMethod.GET)
    public String UsersController.findUsersesByUsername(@RequestParam("username") String username, Model model) {
        model.addAttribute("userses", Users.findUsersesByUsername(username).getResultList());
        return "users/list";
    }
